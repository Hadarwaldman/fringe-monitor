...
async def graphql_with_token(
    ...
    *,
    ...
    owns the token lifecycle. Retries on 429 (single-IP proxy throttling) and
    transient network errors with exponential backoff, since a hold must not
    abort just because a shared-proxy rate limit hit one request."""
    payload: dict[str, Any] = {"query": query}
    if variables is not None:
        payload["variables"] = variables
    ...
    for attempt in range(max_retries):
        try:
            resp = await client.post(f"{API_BASE}/graphql", json=payload, headers=headers)
            if resp.status_code == 429:
                retry_after = resp.headers.get("retry-after")
                delay = (
                    float(retry_after)
    ...
    if last_exc is not None:
        raise last_exc
    raise RuntimeError("edfringe returned 429 repeatedly (proxy rate limited)")
